...
export async function getTransactionsByMonth() {
  await requireAdmin()
  const rows = await db.select().from(transactions).orderBy(desc(transactions.chargedAt))

  const groups = new Map<
    string,
    { key: string; label: string; total: number; entries: number; count: number; rows: TransactionRow[] }
  >()
